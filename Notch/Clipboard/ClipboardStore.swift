...
final class ClipboardStore {
    ...
    private static func supportDirectory() -> URL {
        ...
        let notch = appSupport.appendingPathComponent("Notch", isDirectory: true)
        let legacyPaste = appSupport.appendingPathComponent("Paste", isDirectory: true)
        let fm = FileManager.default
        if !fm.fileExists(atPath: notch.path), fm.fileExists(atPath: legacyPaste.path) {
            try? fm.moveItem(at: legacyPaste, to: notch)
        }
        return notch
    }
